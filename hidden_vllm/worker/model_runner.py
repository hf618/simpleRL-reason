...
from hidden_vllm.worker.model_runner_base import (
    ...
    def execute_model(
        ...
        if self.return_hidden_states:
            ...
            if model_input.is_prompt:
                ...
                if return_decode:
                    hidden_states_decode = [layer_hidden.index_select(0, indices.to(layer_hidden.device)) for layer_hidden in model_executable.model.all_hidden_states]

                if return_prefill:

                # 2. 使用.all_hidden_states 来造（未过RMSNorm）
                    ...
                    if all_hidden_states_from_model:
                        ...
                        states_by_prompt_then_layer = [
                            [states_split_by_layer[layer_idx][prompt_idx] for layer_idx in range(num_layers)]
                        ...
                        for prompt_layers in states_by_prompt_then_layer:
                            # Stack layers for each prompt
                            stacked_tensor = torch.stack(prompt_layers)
                            # Permute dimensions: (num_layers, seq_len, hidden_dim) -> (seq_len, num_layers, hidden_dim)
                            permuted_tensor = stacked_tensor.permute(1, 0, 2)
                            # Left pad if needed
                            padded_tensor = self.pad_tensor_left(
                                permuted_tensor, 
                                target_length=max_seq_len,
                                pad_value=self.pad_token_id
                            )
                            prompt_tensors.append(padded_tensor)

                        # Now all tensors should have the same sequence length
                ...
                if return_decode:
                    # Decode hidden states are already sliced by indices
                    # in the CUDAGraphRunner.capture() method.
                    # So we just need to slice the all_hidden_states.
                    hidden_states_decode = [layer_hidden[:len(indices)] for layer_hidden in model_executable.model.all_hidden_states]
                # output.hidden_states_prefill = None
            else:
                hidden_states = hidden_or_intermediate_states

                if return_decode:
                    hidden_states_decode = [layer_hidden for layer_hidden in model_executable.model.all_hidden_states]
                # output.hidden_states_prefill = None
            # output.hidden_states = hidden_states

            if return_decode:
                output.hidden_states_decode = hidden_states_decode
            model_executable.model.all_hidden_states = []

            output.attn_backend = self.attn_backend
        return [output]
